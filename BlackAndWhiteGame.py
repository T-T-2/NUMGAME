...
# 버전 3 : 게임의 저장 및 기록 기능을 추가했다.

# 게임 기록을 저장할 리스트를 구현한다.
game_history = []
...
while True:
    ...
    if N == '1':
        ...
        for _ in range(10):
            ...
            # 컴퓨터는 카드 더미에서 가장 앞의 카드를 선택한다.(랜덤한 카드를 고르게 하기 위함이다.)
            computer_card = computer_cards.pop(0)

            # 현재 플레이어와 컴퓨터가 낸 카드를 출력한다.
            ...
            # 일반 규칙: 플레이어 카드가 컴퓨터 카드보다 클 경우 플레이어가 승리한다.
            elif player_card > computer_card:
                result[0] += 1  # 승리 횟수 증가
                print("플레이어 승리!")

            # 일반 규칙: 컴퓨터 카드가 플레이어 카드보다 클 경우 컴퓨터가 승리한다.
            elif player_card < computer_card:
                result[2] += 1  # 패배 횟수 증가
                print("컴퓨터 승리!")
    ...
    # 사용자가 "2"를 입력하면 기록 보기 기능을 실행한다.
    elif N == '2':
         if not game_history:
             print("아직 기록된 게임이 없습니다.")
         else:
             print("게임 기록:")
             for i, record in enumerate(game_history, 1):
                print(f"게임 {i}: 승리 {record['승리']}번, 무승부 {record['무승부']}번, 패배 {record['패배']}번")
